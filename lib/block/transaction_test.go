...
func TestMultipleBlockTransactionConfirmed(t *testing.T) {
	...
	var saved []BlockTransaction
	iterFunc, closeFunc := GetBlockTransactionsBySaved(st, nil)
	for {
		bo, hasNext, _ := iterFunc()
		if !hasNext {
			break
...

	{
		// reverse order
		var saved []BlockTransaction
		iterFunc, closeFunc := GetBlockTransactionsBySaved(st, storage.NewDefaultListOptions(true, nil, uint64(numTxs)))
		for {
			bo, hasNext, _ := iterFunc()
			if !hasNext {
				break
